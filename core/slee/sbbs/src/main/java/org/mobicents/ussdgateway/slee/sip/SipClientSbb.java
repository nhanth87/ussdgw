...
public abstract class SipClientSbb extends ChildSbb {
	...
	protected void sendUssdData(XmlMAPDialog xmlMAPDialog) throws Exception {
		...
		case Begin:
			...
			break;
		case Continue:
			UnstructuredSSResponse unstructuredSSResponse = (UnstructuredSSResponse) (xmlMAPDialog.getMAPMessages().isEmpty() ? null : xmlMAPDialog.getMAPMessages().get(0));
	...
	protected Request buildInvite(ScRoutingRule call, XmlMAPDialog xmlMAPDialog) throws Exception {
		...
		MAPMessage mes = mList.isEmpty() ? null : mList.get(0);
		if (mes == null) {
