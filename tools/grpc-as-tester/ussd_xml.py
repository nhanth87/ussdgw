...
def build_push_request_legacy(text: str, invoke_id: int, network_id: int = 0,
    ...
    head = '<dialog type="Begin" appCntx="%s" networkId="%d" mapMessagesSize="1" returnMessageOnError="false"' % (
        APP_CONTEXT, network_id)
    if user_object:
        head += ' userObject="%s"' % _esc(user_object)
    head += ">"
    return ("".join([
        '<?xml version="1.0" encoding="UTF-8"?>',
        head, "<errComponents/>", "<rejectComponents/>", msg, "</dialog>",
    ])).encode("utf-8")
