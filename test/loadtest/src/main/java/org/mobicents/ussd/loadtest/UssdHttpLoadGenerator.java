...
/**
 * High-performance HTTP load generator for USSD Gateway.
 * Target: 10,000 TPS via HTTP interface.
 *
 * Sends ProcessUnstructuredSSRequest as serialized XmlMAPDialog via HTTP POST
 * to the USSD Gateway HTTP endpoint, measures end-to-end latency.
 *
 * Design:
 * - OkHttp with connection pool reuse
 * - N worker threads with precise nanosecond pacing
 * - Lock-free metrics via LoadTestMetrics
 * - Connection pooling to avoid TCP handshake overhead
 */
public class UssdHttpLoadGenerator {

    private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(UssdHttpLoadGenerator.class);
    ...
    public UssdHttpLoadGenerator(String baseUrl, int targetTps, int workerThreads,
        ...
        ConnectionPool connectionPool = new ConnectionPool(
                workerThreads * 2,      // max idle connections
        ...
        // Pre-serialize a template dialog to reduce hot-path allocation
    ...
    public void start() {
        if (running.compareAndSet(false, true)) {
            ...
            logger.info("HTTP LoadGenerator started: threads=" + workerThreads +
    ...
    public void stop() {
        if (running.compareAndSet(true, false)) {
            ...
            logger.info("HTTP LoadGenerator stopped.");
        }
    }
    ...
    private void sendRequest() {
        ...
        // Vary MSISDN slightly per request to avoid cache effects
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 2) {
            ...
            System.out.println("  url           - USSD Gateway HTTP endpoint base URL (e.g. http://localhost:8080/restcomm/)");
        ...
        UssdHttpLoadGenerator generator = new UssdHttpLoadGenerator(url, tps, threads, maxConcurrent, metrics);

        System.out.println("=== USSD HTTP Load Generator ===");
